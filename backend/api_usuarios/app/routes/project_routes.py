...
# GET ALL PROJECTS (solo administradores: lista todos los proyectos del sistema)
@router.get("/projects")
def projects(db: Session = Depends(get_db), current_user: dict = Depends(require_admin)):
...
def project(id: str, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
